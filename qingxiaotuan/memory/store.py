...
class MemoryStore:
    ...
    def _init_db(self) -> None:
        self._db = sqlite3.connect(str(self.dir / "index.db"))
    ...
    def _init_tags_db(self) -> None:
        """初始化标签索引数据库。"""
        self._tags_db = sqlite3.connect(str(self.dir / "tags.db"))
    ...
    def recall_context(self, task: str, limit: int = 5) -> str:
        ...
        for h in fts_hits:
            recalled.append(f"[记忆] {h['content'][:200]}")

        # 最近 24h 的记忆
        recent = self.search_by_time(hours=24, limit=3)
        for r in recent:
            content = r["content"][:200]
            if content not in recalled:
