...
function listPackages(registry) {
...
async function main() {
    ...
    server.register("search", (params) => {
        ...
        const reg = String(params?.registry || DEFAULT_REGISTRY);
        const pkgs = listPackages(reg).filter((p) => p.name.toLowerCase().includes(q) || p.description.toLowerCase().includes(q));
    ...
    server.register("publish", (params) => {
        ...
        if (!meta)
            throw new Error("source has no SKILL.md");
