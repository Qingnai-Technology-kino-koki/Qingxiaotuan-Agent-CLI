...
class Searcher {
    ...
    walk(dir, out) {
        ...
        for (const e of entries) {
            if (this.isIgnored(e.name, path.join(dir, e.name)))
                continue;
    ...
    search(opts) {
        ...
        for (const f of files) {
            if (matches.length >= max)
                break;
            if (globRe && !globRe.test(path.basename(f)))
                continue;
            let content;
            try {
                // 仅读取文本: 探测非打印字符过多则跳过
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                while ((m = re.exec(lineText)) !== null) {
                    if (firstHit === null)
                        firstHit = { s: m.index, e: m.index + m[0].length };
                    if (matches.length >= max)
                        break;
...
async function main() {
    ...
    const server = new protocol_ts_1.IpcServer();
