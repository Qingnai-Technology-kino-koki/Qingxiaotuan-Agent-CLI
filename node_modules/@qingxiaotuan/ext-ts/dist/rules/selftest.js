...
- id: no-print
  ...
  message: 不要用print
`;
const rules = (0, main_1.parseRules)(yaml);
...
const engine = new main_1.RuleEngine();
...
check("app.py 无 TODO 不触发 no-todo", !v2.some((x) => x.id === "no-todo"));
// 4) 非 .py 文件不匹配 path
const v3 = engine.check("app.js", "# TODO x", "file");
check("app.js 即使含TODO也不匹配 *.py", !v3.some((x) => x.id === "no-todo"));
// 5) warn 规则: print( 触发
const v4 = engine.check("lib.py", "print('hi')\n", "file");
check("print( 触发 no-print warn", v4.some((x) => x.id === "no-print"));
...
- id: needs-body
  ...
  message: 内容不能为空
`;
const eng2 = new main_1.RuleEngine();
eng2.loadYamlText(secYaml);
const s1 = eng2.check("cfg.env", "API_KEY=abcdef0123456789abcdef0123456789\n", "file");
check("has_secret 命中长十六进制密钥", s1.some((x) => x.id === "no-secret"));
const s2 = eng2.check("run.js", "const x = eval(userInput);\n", "file");
check("forbidden 命中 eval(", s2.some((x) => x.id === "no-eval"));
const s3 = eng2.check("big.txt", "a".repeat(200), "file");
check("max_len 超过100触发 warn", s3.some((x) => x.id === "max-len"));
const s4 = eng2.check("empty.txt", "", "file");
check("required 对空内容触发 error", s4.some((x) => x.id === "needs-body"));
...
const badYaml = `- id: x
  ...
  message: m
`;
const eng3 = new main_1.RuleEngine();
eng3.loadYamlText(badYaml);
const vres = eng3.validate();
check("validate 报告非法 severity", vres.valid === false && vres.issues.some((i) => i.includes("severity")));
