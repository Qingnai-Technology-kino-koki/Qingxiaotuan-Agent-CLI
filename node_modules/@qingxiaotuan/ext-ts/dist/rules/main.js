...
function evalExpr(node, scope) {
    ...
    if (node.call) {
        const a = node.args.map((x) => evalExpr(x, scope));
        switch (node.call) {
...
class RuleEngine {
    ...
    validate() {
        ...
        for (const r of this.rules) {
            ...
            if (!r.message)
                issues.push(`规则 ${r.id} 缺少 message`);
        }
        return { valid: issues.length === 0, issues };
    }
    /** 返回命中的行号 (1-based) 用于上下文展示。 */
    hitLines(content, needle) {
        const out = [];
        ...
        for (const r of this.rules) {
            ...
            try {
                ...
                if (!ok) {
                    ...
                    if (m)
                        lines = this.hitLines(content, m[1]);
                    violations.push({
...
function parseRules(text) {
    ...
    return rules.filter((r) => r.id && r.assert).map((r) => {
        ...
        if (typeof r.match === "string") {
            nr.match = { content: r.match };
        }
        else if (r.match && typeof r.match === "object") {
            nr.match = r.match;
        }
        return nr;
    });
...
async function main() {
    ...
    const server = new protocol_1.IpcServer();
    server.register("load", (params) => {
        let count = 0;
        if (params?.rules_yaml)
            count = engine.loadYamlText(String(params.rules_yaml));
        else if (params?.rules_path)
            count = engine.loadPath(String(params.rules_path));
        else
            throw new Error("missing rules_yaml or rules_path");
    ...
    server.register("validate", (params) => {
        if (params?.rules_yaml)
            engine.loadYamlText(String(params.rules_yaml));
