...
const ctx = { home: "", port: 18765, server: null, wss: null };
function readSessions(home) {
    ...
    for (const f of fs.readdirSync(dir)) {
        ...
        for (const line of text.split("\n")) {
            ...
            try {
                ...
                if (rec.type === "user") {
                    turns++;
                    lastUser = rec.message?.content || "";
                }
                else if (rec.type === "assistant")
                    lastAssistant = rec.message?.content || "";
            }
            catch { /* ignore */ }
        }
...
function startServer(home, port) {
    ...
    setInterval(() => {
        if (wss.clients.size > 0) {
            ...
            wss.clients.forEach((c) => { if (c.readyState === ws_1.WebSocket.OPEN)
                c.send(snap); });
        }
    }, 3000);
}
